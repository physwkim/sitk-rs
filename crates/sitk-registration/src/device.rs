...
impl DeviceMeanSquaresMetric {
    ...
    pub fn from_device(
        ...
        Ok(Self {
            resident: ResidentMetric::from_device(fixed, points, moving, &geom)?,
        })
    }

    /// Number of fixed samples — every voxel of the fixed image.
    pub fn sample_count(&self) -> usize {
        self.resident.sample_count()
    }
    ...
    pub fn evaluate(
        ...
        let moments = self.resident.evaluate(&form.a, &form.b)?;
    ...
    pub fn value(&mut self, transform: &dyn ParametricTransform) -> Result<f64, DeviceMetricError> {
