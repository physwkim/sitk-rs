...
use sitk_core::Image;
use sitk_registration::{CpuBackend, MeanSquaresMetric};
...
fn arith(threads: usize) {
    ...
    let mut sum = 0.0f64;
    for _ in 0..iters {
        sitk_core::parallel::map_rows_fold_in_order(
            n,
            7,
            || (),
            |(), s, row| {
...
fn phases(threads: usize) {
    ...
    for _ in 0..iters {
        ...
        let t = Instant::now();
        let rows: Vec<[f64; 7]> = sitk_core::parallel::map_indexed(n, |s| {
            let x = (s % FIXED) as f64;
            let y = ((s / FIXED) % FIXED) as f64;
            let z = (s / (FIXED * FIXED)) as f64;
...
fn phases_reuse(threads: usize, mult: usize) {
    ...
    for _ in 0..iters {
        let t = Instant::now();
        sitk_core::parallel::map_indexed_into(&mut rows, |s| {
            let x = (s % FIXED) as f64;
            let y = ((s / FIXED) % FIXED) as f64;
            let z = (s / (FIXED * FIXED)) as f64;
            let mut row = [0.0f64; 7];
...
fn straggler(threads: usize) {
    ...
    for _ in 0..iters {
        ...
        let times: Vec<f64> = sitk_core::parallel::map_indexed(threads, |w| {
            let t = Instant::now();
            let mut acc = 0.0f64;
            for j in 0..per {
                let s = w * per + j;
