...
use sitk_cuda::{CudaError, backend};
...
fn the_summation_order_is_visible_on_this_input() {
    ...
    let forward = sitk_cuda::histogram_host(&keys, &vals, nbins);
...
fn the_device_histogram_is_the_host_histogram_bit_for_bit() {
    ...
    for (n, nbins) in [
        ...
        let host = sitk_cuda::histogram_host(&keys, &vals, nbins);
        let device = sitk_cuda::histogram(&keys, &vals, nbins).expect("device histogram");
...
fn the_device_histogram_is_deterministic_run_to_run() {
    ...
    let first = sitk_cuda::histogram(&keys, &vals, nbins).expect("device histogram");
    for run in 1..8 {
        let again = sitk_cuda::histogram(&keys, &vals, nbins).expect("device histogram");
...
fn the_result_does_not_depend_on_the_launch_configuration() {
    ...
    let reference = sitk_cuda::histogram(&keys, &vals, nbins).expect("device histogram");
    for block in [32usize, 64, 128, 256, 512, 1024] {
        let got =
            sitk_cuda::histogram_with_block(&keys, &vals, nbins, block).expect("device histogram");
...
fn the_atomic_histogram_is_not_deterministic_and_that_is_why_this_module_exists() {
    ...
    let first = sitk_cuda::histogram_atomic(&keys, &vals, nbins).expect("atomic histogram");
    let mut runs_differing = 0usize;
    let mut worst_bins = 0usize;
    let mut worst_rel = 0.0f64;
    for _ in 1..8 {
        let again = sitk_cuda::histogram_atomic(&keys, &vals, nbins).expect("atomic histogram");
        let differ = bits_differ(&first, &again);
        if differ > 0 {
            runs_differing += 1;
...
fn a_malformed_entry_list_is_refused() {
    ...
    for (what, keys, vals, nbins) in [
        ...
        match sitk_cuda::histogram(&keys, &vals, nbins) {
