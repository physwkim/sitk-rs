...
//! The rescale itself, `RescaleFunction` (:239-247), is applied to the first
//! `numElts` **components** of the buffer, where `numElts` is the *voxel*
//! count — see ledger §1.46 for what that does to a multi-component image.
//!
...
//!   compression dependency (ledger §4.53, §5.8).
...
//!   little-endian host (ledger §4.49).
...
/// is why ledger §4.48 records that the "very similar" comparison could disagree
...
/// `IMG.NII` before reaching here (ledger §2.82).
...
/// the variable once, in the constructor; this reads it per call (ledger §4.51), because the
...
impl Info {
    ...
    /// before we get here (ledger §3.30).
...
/// notation, where double-conversion does outside `1e-6 .. 1e21` (ledger §4.54).
...
/// :630, so a NIfTI image never carries it. Ledger §2.77.
///
...
/// block for the matrix). This port's dictionary is `String`-valued. Ledger §4.47.
        ...
        NIFTI_INTENT_SYMMATRIX => {
            return Err(IoError::UnsupportedNiftiFeature(format!(
                "{}: NIFTI_INTENT_SYMMATRIX loads as itk::IOPixelEnum::SYMMETRICSECONDRANKTENSOR, \
                 which SimpleITK's GetPixelIDFromImageIO rejects (ledger §3.30)",
                path.display()
            )));
        }
...
/// `count` components of the buffer.
///
/// `count` is `numElts` — the **voxel** count, not the component count — so on
/// a multi-component image only a prefix of the buffer is rescaled. That is
/// upstream's behaviour verbatim (ledger §1.46). The integer arms of upstream's
/// dispatch (:518-548) are dead: `ReadImageInformation` has already promoted
/// every integer component type to `float`.
...
/// Reproduced (it is memory-safe; ledger §1.47).
...
pub fn read(path: &Path) -> Result<Image> {
    ...
    if promoted {
        if info.kind == PixelKind::Vector {
            ...
            // per *component*. Both overrun the heap (ledger §1.45); there is no
            ...
            return Err(IoError::UnsupportedNiftiFeature(format!(
                "{}: a vector NIfTI image with an integer datatype and a non-trivial \
                 ...
                 (ledger §1.45/§4.50); refusing to guess what it meant",
                path.display()
            )));
        }
...
/// carrying `qform_code = "abc"` fails the write (ledger §2.79).
...
fn write_information(image: &Image, path: &Path) -> Result<WriteInfo> {
    ...
    // refused (ledger §2.82). `IsCompressed` likewise looks for a lowercase
    // `.gz` only (:1173).
