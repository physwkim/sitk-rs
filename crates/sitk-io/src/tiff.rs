...
//! refused rather than round-tripped approximately (ledger §4.95); the signed
...
//! `.cxx` at all. Ledger §2.136; [`page_rows`] reproduces it by taking the
...
//! precisely the set upstream reads out of bounds — is refused. Ledger §1.65,
//! §4.93.
//!
...
//! `m_XResolution > 0` passes on `Clean()`'s seed. Ledger §2.137.
//!
...
//! leaves its constructed default. Ledger §3.49.
//!
...
//! `9` (itkImageIOBase.h:830). Ledger §3.50.
//!
...
/// That stale-state bug (ledger §1.64) has no analogue here — this function is
        ...
        (32, other) => {
            return unsupported(format!(
                "TIFF SampleFormat {other} at 32 bits per sample leaves m_ComponentType \
                 unassigned upstream (itkTIFFImageIO.cxx:406-420) — doc/upstream-findings.md §1.64"
            ));
        }
        (_, 2) => PixelId::Int16,
        _ => PixelId::UInt16,
    ...
    if page0.tiled {
        return unsupported(
            "tiled TIFF images are read through TIFFReadRGBAImageOriented upstream \
             (itkTIFFReaderInternal.cxx:281) — doc/upstream-findings.md §4.96",
        );
    }
    ...
    if page0.orientation != ORIENTATION_TOPLEFT && page0.orientation != ORIENTATION_BOTLEFT {
        return unsupported(format!(
            "TIFF Orientation {} is read through TIFFReadRGBAImageOriented upstream \
             (itkTIFFReaderInternal.cxx:288) — doc/upstream-findings.md §4.96",
            page0.orientation
        ));
    }
    if !matches!(page0.bits_per_sample, 8 | 16 | 32) {
        return unsupported(format!(
            "TIFF BitsPerSample {} is read through TIFFReadRGBAImageOriented upstream \
             (itkTIFFReaderInternal.cxx:289) — doc/upstream-findings.md §4.96",
        ...
        other => {
            return unsupported(format!(
                "PhotometricInterpretation::{other:?} is read through TIFFReadRGBAImageOriented \
                 upstream (itkTIFFReaderInternal.cxx:284-286) — doc/upstream-findings.md §4.96"
            ));
        }
    };
    ...
    if photometric == PhotometricInterpretation::WhiteIsZero && component == PixelId::Float32 {
        return unsupported(
            "the `tiff` crate inverts a 32-bit-float MINISWHITE image with `1.0 - v`, which \
             ...
             recovered — doc/upstream-findings.md §4.95",
        );
    }
            ...
            if named != stored {
                return unsupported(format!(
                    "an RGB TIFF with SamplesPerPixel {stored} decodes as {named} samples in the \
                     ...
                     (itkTIFFImageIO.cxx:441-444) — doc/upstream-findings.md §4.96"
                ));
            }
            stored
        }
...
/// field registry, which the `tiff` crate does not expose (§4.92).
...
/// with the per-page geometry check that closes §1.65.
...
mod tests {
    ...
    /// every decode path goes through it. Ledger §4.94.
    #[test]
    ...
    fn a_volume_whose_pages_differ_in_geometry_is_refused() {
        ...
        assert!(
            matches!(&image, Err(IoError::UnsupportedTiffFeature(m)) if m.contains("§1.65")),
            "{image:?}"
        );
    }
    ...
    /// no `default:` (§1.64) and the `bits > 32` fall-through to `USHORT`.
    #[test]
    ...
    fn double_to_rational_matches_libtiff() {
        ...
        // 1.0000000150184933 (itkTIFFImageIO.cxx:378, §2.138).
