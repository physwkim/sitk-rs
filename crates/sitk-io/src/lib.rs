...
//! `itk::ImageIOFactory`. Adding NRRD, PNG or DICOM later is a new module plus
...
//!
//! Two uncompressed formats are supported:
//!
...
//!   pixel type, vector images, and — unlike MetaImage — complex images as
//!   complex. `.nii.gz` is recognised and rejected; this workspace has no zlib.
...
mod tests {
    ...
    fn registry_lists_each_image_io_by_class_name() {
        assert_eq!(registered_image_ios(), vec!["MetaImageIO", "NiftiImageIO"]);
        assert_eq!(
        ...
        assert!(matches!(
            image_io_by_name("NrrdImageIO"),
            Err(IoError::UnknownImageIo(name)) if name == "NrrdImageIO"
        ));
    }
    ...
    /// 1-D. Upstream quirk, pinned (ledger §2.80).
    #[test]
    ...
    /// the second half get neither. Upstream bug, reproduced (ledger §1.46).
    #[test]
    ...
    /// every MetaImage does. Upstream quirk, pinned (ledger §2.77).
    #[test]
    ...
    /// (nifti1_io.c:3452-3454). Upstream quirk, pinned (ledger §2.83).
    #[test]
    ...
    /// simply cannot open such a file (ledger §3.30).
    #[test]
    ...
    /// reproduce, so the read is refused (ledger §1.45, §4.50).
    #[test]
