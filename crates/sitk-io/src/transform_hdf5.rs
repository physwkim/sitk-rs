...
//! number of subgroups (ledger §2.122).
//!
//! # Fidelity notes
//!
...
//! port always writes contiguous (ledger §3.43, §5.22).
//!
...
pub(crate) fn read_transform_list(path: &Path) -> Result<Vec<Transform>> {
    ...
    // the subgroups the writer put there (ledger §2.122).
...
/// empty release (ledger §4.83). No reader consumes either.
...
mod tests {
    ...
    /// contiguous (ledger §3.43, §5.22). Nothing in SimpleITK sets the flag, but
    ...
    /// (ledger §2.122). ITK dies inside `openGroup`; here it is a `NotFound`.
    #[test]
    ...
    /// throws (ledger §2.123). Here `has_group` is consulted, so the file is
    /// simply not claimed.
    #[test]
