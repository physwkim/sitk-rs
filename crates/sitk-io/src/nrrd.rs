...
//! `spaceDim == 0`, and which reproduces upstream bug [§1.46](
...
//! vectors are used unconverted — see ledger §2.77. The same sign flips are
...
//! has no pixel id for and rejects — ledger §3.30.
//!
//! # Encodings
//!
...
//!   that array when `dim == 16` (ledger §1.45, §4.46).
...
//!   bare-filename and `LIST` forms only. Ledger §4.48.
...
//!   `nrrdSpacingCalculate`. Same predicate, earlier. Ledger §4.49.
//!
//! # Reproduced upstream quirks
//!
//! * `kinds: none` assigns to the axis's **centering**, not its kind
//!   (parseNrrd.c:621-623) — ledger §1.47.
    ...
    match id {
        ...
        F_KINDS => {
            ...
            for (i, token) in tokens.iter().enumerate() {
                ...
                if *token == "none" {
                    // Upstream bug (ledger §1.47): the `none` arm assigns to
...
fn parse_data_file(
    ...
    if let Some(after) = info.strip_prefix("LIST") {
        ...
        // filename rather than a terminator (ledger §2.78).
...
/// if-ladder and raises "Unknown PixelType" (ledger §3.30).
...
fn image_information(nrrd: &Nrrd) -> Result<Information> {
    ...
    // well-defined LPS conversion are simply left alone (ledger §2.77).
...
/// §1.46, reproduced. A header with `axis mins: 0 nan` therefore reports
...
/// parenthesised column spelling for the frame. Ledger §4.47.
fn build_metadata(
    nrrd: &Nrrd,
    order: &AxisOrder,
...
fn build_header(nrrd: &Nrrd, io: &IoState) -> String {
    ...
    // key/value pairs anyway, which corrupts such a header (ledger §2.79);
...
/// `(a,b,c) (d,e,f) ...` column spelling (ledger §4.47).
...
mod tests {
    ...
    fn kinds_none_clobbers_centering_not_kind() {
        // Upstream bug §1.47: `kinds: ... none` writes `.center`.
