...
impl ThreadConfig {
    ...
    /// `sitk::core::parallel::with_threads` is the port's designated t1/tN
...
fn main() {
    ...
    for &(size_name, dim) in SIZES {
        ...
        for op in OPS {
            ...
            // Determinism contract (`sitk::core::parallel` module docs): every
            ...
            if op.key == "rescale_intensity" {
                #[cfg(feature = "cuda")]
                {
                    match sitk::cuda::rescale_intensity_gpu(
                        input,
                        RESCALE_OUTPUT_MIN,
                        RESCALE_OUTPUT_MAX,
                    ) {
                        Ok((gpu_output, _timings)) => {
                            ...
                            criterion.bench_function(&bench_id, |b| {
                                b.iter(|| {
                                    sitk::cuda::rescale_intensity_gpu(
                                        black_box(input),
                                        RESCALE_OUTPUT_MIN,
                                        RESCALE_OUTPUT_MAX,
                                    )
...
/// op, and `smooth_gaussian`, which is **not** — it is `sitk::filters::smooth_gaussian`
...
fn gpu_resident_row(
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let device_input = match sitk::cuda::DeviceImage::upload(input) {
            Ok(d) => d,
            Err(e) => {
                row(
                    None,
        ...
        let (output_checksum, errs) = match sitk::cuda::rescale_intensity(
            &device_input,
            RESCALE_OUTPUT_MIN,
            RESCALE_OUTPUT_MAX,
        )
