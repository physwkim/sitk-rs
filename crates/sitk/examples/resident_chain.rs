...
use sitk::core::Image;
#[cfg(feature = "cuda")]
use sitk::cuda::{DeviceBuffer, backend, rescale_intensity_gpu, rescale_intensity_resident};
#[cfg(feature = "cuda")]
use sitk::registration::{CpuBackend, CudaMetricBackend, MeanSquaresMetric, MetricBackend};
#[cfg(feature = "cuda")]
...
fn main() {
    ...
    sitk::core::parallel::with_threads(threads, || {
        ...
        let cpu_op = median(
            (0..5)
                .map(|_| {
                    let t = Instant::now();
                    std::hint::black_box(
                        sitk::filters::rescale_intensity(&fixed_raw, OUT_MIN, OUT_MAX).unwrap(),
                    );
                    ms(t)
                })
                .collect(),
        ...
        let mut host_back = sitk::core::alloc::resident_vec::<f32>(voxels);
        let d2h = median(
            (0..5)
                .map(|_| {
                    let t = Instant::now();
        ...
        let t = Instant::now();
        let f = sitk::filters::rescale_intensity(&fixed_raw, OUT_MIN, OUT_MAX).unwrap();
        let m = sitk::filters::rescale_intensity(&moving_raw, OUT_MIN, OUT_MAX).unwrap();
