...
fn main() {
    ...
    sitk::core::parallel::with_threads(threads, || {
        ...
        let host_chain = |img: &Image, stage: &mut [f64; 3]| {
            let t = Instant::now();
            let c = sitk::filters::cast(img, PixelId::Float32).unwrap();
            stage[0] += ms(t);
            let t = Instant::now();
            let r = sitk::filters::rescale_intensity(&c, OUT_MIN, OUT_MAX).unwrap();
            stage[1] += ms(t);
            let t = Instant::now();
            let s = sitk::filters::smooth_gaussian(&r, &SIGMA).unwrap();
            stage[2] += ms(t);
            s
        };
