...
use sitk::registration::ImageRegistrationMethod;
...
fn an_integer_pyramid_is_the_pyramid_of_its_float_cast() {
    ...
    let ffix = sitk::filters::cast(&fixed, PixelId::Float32).unwrap();
    let fmov = sitk::filters::cast(&moving, PixelId::Float32).unwrap();
...
fn the_device_pyramid_agrees_with_the_host_pyramid_on_an_integer_volume() {
    use sitk::cuda::{CudaError, DeviceImage};

    if matches!(sitk::cuda::backend(), Err(CudaError::NoDevice(_))) {
