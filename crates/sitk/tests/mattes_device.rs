...
fn a_moving_mask_narrows_the_moving_axis_on_the_device_too() {
    ...
    let mut mask = vec![false; N * N * N];
    {
        let buf: &mut Vec<f32> = match moving.buffer_mut() {
            sitk::core::PixelBuffer::Float32(b) => b,
    ...
    let masked = {
        let f = sitk::registration::metric::FixedSamples::from_image(&fixed).unwrap();
        let m = sitk::registration::metric::MovingImage::from_image(&moving)
            .unwrap()
            .with_moving_mask(
                &Image::from_vec(
                    &[N, N, N],
