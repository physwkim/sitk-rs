...
fn a_mask_that_admits_no_voxels_is_refused_by_name() {
    ...
    assert!(
        matches!(
            err,
            sitk::filters::FilterError::MaskAdmitsNoVoxels { mask_value: 255 }
        ),
        "expected MaskAdmitsNoVoxels, got {err:?}"
    );
    let err = huang_threshold(&img, 128, 1, 0, Some(&mask)).unwrap_err();
    assert!(
        matches!(err, sitk::filters::FilterError::MaskAdmitsNoVoxels { .. }),
...
fn a_mask_of_a_different_size_is_an_error() {
    ...
    assert!(matches!(
        ...
        Err(sitk::filters::FilterError::SizeMismatch { .. })
    ));
}
...
fn a_mask_on_a_different_grid_is_refused() {
    ...
    assert!(matches!(
        ...
        Err(sitk::filters::FilterError::PhysicalSpaceMismatch { index: 1 })
    ));
...
fn a_mask_that_is_not_uint8_is_refused_by_name() {
    ...
    assert!(
        matches!(
            err,
            sitk::filters::FilterError::RequiresUInt8MaskPixelType(sitk::core::PixelId::Float64)
        ),
