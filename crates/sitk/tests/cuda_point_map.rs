...
use sitk::registration::device::DeviceMetricError;
use sitk::registration::metric::{FixedSamples, MovingImage};
use sitk::registration::{DeviceMeanSquaresMetric, MeanSquaresMetric};
...
fn the_cpu_still_evaluates_what_the_device_refuses() {
    ...
    .unwrap();
    let scale = ScaleTransform::new(vec![1.1, 0.9, 1.05], vec![8.0, 8.0, 8.0]);
    let v = m.evaluate(&scale, &sitk::registration::CpuBackend);
