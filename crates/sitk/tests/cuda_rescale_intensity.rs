...
use sitk::filters::{FilterError, rescale_intensity, rescale_intensity_cpu};
...
fn gpu_matches_cpu_on_the_medium_volume() {
    ...
    let (gpu, cold) = match sitk::cuda::rescale_intensity_gpu(&img, 0.0, 255.0) {
        Ok(v) => v,
        ...
        Err(e @ sitk::cuda::CudaError::NoDevice(_)) => {
    ...
    let mut warm: Vec<sitk::cuda::GpuTimings> = (0..5)
        .map(|_| {
            sitk::cuda::rescale_intensity_gpu(&img, 0.0, 255.0)
                .unwrap()
                .1
        })
        .collect();
    ...
    let pooled_d2h = {
        let backend = sitk::cuda::backend().unwrap();
        let d = sitk::cuda::DeviceBuffer::from_host(backend, img.scalar_slice::<f32>().unwrap())
            .unwrap();
...
fn rescale_intensity_baseline_cpu_vs_gpu_interleaved() {
    ...
    for (label, n) in [("small", 64usize), ("medium", 256), ("large", 512)] {
        ...
        let med = |f: fn(&sitk::cuda::GpuTimings) -> f64| median(gpu_t.iter().map(f).collect());
...
/// caller of `sitk::filters::rescale_intensity` gets with the feature on.
#[test]
