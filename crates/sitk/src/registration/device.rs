...
//!
//! [`crate::registration::CudaMetricBackend`] takes host images and uploads them. When those
...
//!
//! [`CudaMetricBackend`](crate::registration::CudaMetricBackend) is infallible by signature and
...
use crate::transform::interpolator::{index_to_physical_matrix, physical_to_index_matrix, strides};
use crate::transform::{Interpolator, ParametricTransform};
use thiserror::Error;

use crate::registration::cuda::{
    affine_form, contract, contract_correlation, contract_mattes, point_stages,
};
use crate::registration::mattes::{MattesGeometry, MattesTail, mattes_tail};
use crate::registration::metric::MetricValue;
use crate::registration::scales::{ScalesEstimator, ScalesEstimatorKind, VirtualGrid};
...
pub enum DeviceMetricError {
    ...
    /// reports no [`point_map_stages`](crate::transform::TransformBase::point_map_stages).
    ///
    ...
    #[error(transparent)]
    MattesGeometry(#[from] crate::registration::RegistrationError),
...
/// [`ImageRegistrationMethod::execute`](crate::registration::ImageRegistrationMethod::execute) on
...
pub enum DeviceRegistrationError {
    ...
    /// that ([`crate::cuda::resample_linear_through`] /
    /// [`crate::cuda::resample_nearest_through`]) by replaying the transform's **own
    /// stages** ([`point_map_stages`](crate::transform::TransformBase::point_map_stages)),
    ...
    UnsupportedFixedInitialTransform(crate::transform::TransformKind),
...
impl DeviceMeanSquaresMetric {
    ...
    pub fn from_device_masked(
        fixed: &DeviceImage,
        moving: &DeviceImage,
        fixed_mask: Option<&crate::cuda::DeviceMask>,
    ...
    /// [`crate::registration::metric::draw_samples`]'s — the same function, called with the same
    ...
    pub fn evaluate(
        ...
        if transform.dimension() != crate::cuda::DIM {
...
/// [`crate::cuda::ResidentCorrelation`] and the `one_pass_moment_form` tests in
/// [`crate::registration::correlation`] — because it trades the host's stable `Σ(f−f̄)²` for
...
///
/// [`CorrelationMetric::check_transform`](crate::registration::CorrelationMetric::check_transform)
...
///
/// The joint Parzen histogram comes off [`crate::cuda::ResidentMattes`], which builds it
...
///
/// [`ParametricTransform::jacobian_wrt_parameters`]: crate::transform::ParametricTransform::jacobian_wrt_parameters
pub struct DeviceMattesMetric {
    /// See [`DeviceMeanSquaresMetric::resident`] — same reason, same shape.
    resident: Mutex<crate::cuda::ResidentMattes>,
...
impl DeviceMattesMetric {
    ...
    pub fn from_device_sampled(
        ...
        let (resident, layout) =
            with_device_layout(fixed, moving, moving_mask, samples, |points, geom| {
                crate::cuda::ResidentMattes::from_device_masked(
    ...
    fn lock(&self) -> std::sync::MutexGuard<'_, crate::cuda::ResidentMattes> {
        self.resident
            .lock()
            .expect("the device metric's resident buffers are poisoned by an earlier panic")
    }
...
/// signature ([`crate::registration::optimizer::Objective`] returns values, not `Result`s).
///
...
/// boundary in
/// [`ImageRegistrationMethod::execute_on_device`](crate::registration::ImageRegistrationMethod::execute_on_device),
