...
//! metrics ([`crate::registration::MeanSquaresMetric`], [`crate::registration::MattesMutualInformationMetric`]) use.
//!
//! ## Parity notes vs ITK
//!
...
//!   [`mattes`](crate::registration::mattes): a global transform (translation, affine,
...
//!   branch). A [local-support](crate::transform::ParametricTransform::has_local_support)
//!   [`crate::transform::DisplacementFieldTransform`] instead writes each point's contribution
//!   directly into *its own* parameter block via the block that
//!   `metric::local_support_block` assembles from
//!   [`sparse_jacobian_wrt_parameters`](crate::transform::ParametricTransform::sparse_jacobian_wrt_parameters)
...
//!   [`mattes`](crate::registration::mattes).

use crate::core::{Image, coord};
use crate::transform::ParametricTransform;
use crate::transform::interpolator::{physical_to_index_matrix, strides};

use crate::registration::error::{RegistrationError, Result};
use crate::registration::metric::{
    FixedSamples, MetricValue, MovingImage, PointScratch, local_support_block,
};
...
pub struct AntsNeighborhoodCorrelationMetric {
    ...
    /// row-major order (see [`crate::transform::interpolator::strides`]).
...
impl AntsNeighborhoodCorrelationMetric {
    ...
    /// [`mattes`](crate::registration::mattes) and ITK's `HasLocalSupport()` do: a global
