...
//! [`mattes`](crate::registration::mattes) module and reuses this module's [`FixedSamples`] /
...
//! construction, from [`crate::core::parallel::map_rows_fold_in_order`]: the
...
use crate::core::{Image, Scalar, coord, dispatch_scalar};
...
///
/// Shared with [`crate::registration::scales`], whose `Sampling::Random` strategy stands in
...
/// `crate::core::fused::map_pixels` keep bit-parity while deleting the port's
...
pub struct FixedSamples {
    ...
    /// [`points`](Self::points) (see [`crate::registration::scales`]). Keeping the geometry
...
impl MovingImage {
    ...
    /// [`crate::transform::Interpolator`]). Fails if its direction matrix is
    /// singular.
    ...
    /// joint histogram. Both callers — [`crate::registration::mattes`] and
    /// [`crate::registration::joint_histogram`] — turn it into a bin size, a normalized
...
///
/// Seeded and folded exactly as [`crate::core::parallel::min_max`]'s serial scan is
...
impl MetricBackend for CpuBackend {
    fn mean_squares(
        ...
        if sparse {
            ...
            // the thread count. See `crate::core::parallel::map_rows_fold_in_order`.
...
mod tests {
    use super::*;
    use crate::transform::{Euler3DTransform, TransformBase, TranslationTransform};
    ...
    fn mean_squares_is_bit_identical_to_the_serial_loop_at_every_thread_count() {
        ...
        for threads in [1usize, 2, 3, 8, 32] {
            let (got, got_only) = crate::core::parallel::with_threads(threads, || {
                (
