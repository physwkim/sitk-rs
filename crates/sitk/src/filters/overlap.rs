...
//! a [`crate::filters::distance::signed_maurer_distance_map`] of the *second* input
...
//! ([`crate::core::compensated::CompensatedSum`]). It did not until 2026-07-15: it used a
...
//! pointed at prove it. [`crate::filters::statistics`] had the **same defect** — its upstream,
//! `itkStatisticsImageFilter`, compensates both of its accumulators — and is fixed in the
//! same sweep. [`crate::filters::label::label_statistics`] is **correct as it stands**, because
...
use crate::filters::distance::signed_maurer_distance_map;
...
mod tests {
    ...
    fn the_average_distance_is_compensated_and_a_naive_walk_is_not_the_same_number() {
        ...
        let reference_average = crate::core::compensated::neumaier_sum(terms) / count as f64;
