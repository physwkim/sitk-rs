...
impl<'a> ThresholdMask<'a> {
    ...
    fn selected(&self, img: &Image, vals: &[f64]) -> Result<Vec<f64>> {
        ...
        // [`crate::filters::mask_input`], which owns them for every mask input in this crate.
        let mask = crate::filters::mask_input::uint8_mask_voxels(img, self.image)?;
...
impl Histogram {
    ...
    /// [`crate::filters::threshold::isodata_threshold`] as the fallback when no
