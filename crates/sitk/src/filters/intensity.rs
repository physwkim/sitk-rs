...
//!   (this crate's [`crate::filters::statistics`] is already verified against
...
//!
//! See [`crate::filters::histogram`] for the histogram construction convention shared
...
use crate::filters::functor::UnaryFunctor;
...
functor::unary_functor! {
    ...
    /// the corresponding output bound. Unlike [`crate::filters::rescale_intensity`],
...
pub fn normalize(img: &Image) -> Result<Image> {
    let stats = crate::filters::statistics(img)?;
    ...
    let vals: Vec<f64> = img
        ...
        .map(|&v| (v + shift) * scale)
        .collect();
    crate::filters::image_from_f64(real_type(img.pixel_id()), img.size(), img, &vals)
}
...
pub fn shift_scale(
    ...
    let (max_value, nonpositive_min) = crate::filters::morphology::bounds_for(output_id);
    ...
    let image = crate::filters::image_from_f64(output_id, img.size(), img, &vals)?;
    Ok(ShiftScaleResult {
        image,
        underflow_count,
        overflow_count,
...
pub fn normalize_to_constant(img: &Image, constant: f64) -> Result<Image> {
    let stats = crate::filters::statistics(img)?;
    let output_id = crate::filters::real_pixel_id(img.pixel_id());
    ...
    crate::filters::image_from_f64(output_id, img.size(), img, &vals)
}
