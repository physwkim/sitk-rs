...
//! Triangle, which live in [`crate::filters::intensity`] alongside the histogram
...
//!
//! Every calculator here shares [`crate::filters::intensity::otsu_threshold`]'s
...
/// pixels inside pass through unchanged. Unlike [`crate::filters::binary_threshold`],
...
/// [`crate::filters::binary_threshold`] already does), which only differs from
...
pub fn threshold(img: &Image, lower: f64, upper: f64, outside_value: f64) -> Result<Image> {
    ...
    let out: Vec<f64> = vals
        ...
        .collect();
    crate::filters::image_from_f64(img.pixel_id(), img.size(), img, &out)
}
...
/// bin, every pixel clips into the last bin (see [`crate::filters::histogram`]), so
...
/// single populated bin — see [`crate::filters::histogram`]) never becomes bimodal no
...
/// mass in the last bin, see [`crate::filters::histogram`]), the first pass's rounded
...
mod tests {
    ...
    fn huang_threshold_constant_image_returns_the_constant_value() {
        // first_bin == last_bin on a constant image (see crate::filters::histogram),
