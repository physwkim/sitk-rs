...
//!
//! Unlike the FIR [`smooth_gaussian`](crate::filters::smooth_gaussian) — which samples
...
//! [`crate::core::deriche::Coefficients`] — ITK's `SetUp` / `ComputeDCoefficients`
...
//! an [`AxisTooShortForRecursion`](crate::filters::FilterError::AxisTooShortForRecursion)
//! error.
//!
...
use crate::core::deriche::Coefficients;
use crate::core::{Image, PixelId, parallel};
use crate::filters::error::{FilterError, Result};
use crate::filters::image_from_f64;

pub use crate::core::deriche::GaussianOrder;
...
/// quantization step — [`crate::filters::recursive_gaussian::smoothing_recursive_gaussian`]'s
/// `RebindImageType<float>` output and
/// [`crate::filters::gradient::gradient_recursive_gaussian`]'s per-axis derivative
...
/// a kernel lock, which is why [`crate::core::parallel::map_indexed`]'s allocating
...
/// document against that family; this filter (and [`crate::filters::gradient::gradient`]/
/// [`crate::filters::gradient::gradient_recursive_gaussian`], which share the same
...
/// reproduces that literally: [`crate::core::Image::extract_component`] each
...
/// [`crate::core::Image::from_component_images`] them back together. A complex
/// image is rejected the same way [`crate::filters::dicom_orient::dicom_orient`]'s
...
mod tests {
    ...
    fn approximates_the_fir_gaussian_on_a_smooth_blob() {
        ...
        let rec = recursive_gaussian(&img, &[2.0, 2.0])
            ...
            .to_f64_vec()
            .unwrap();
        let fir = crate::filters::smooth_gaussian(&img, &[2.0, 2.0])
            .unwrap()
            .to_f64_vec()
            .unwrap();
    ...
    fn smoothing_recursive_gaussian_scalar_matches_recursive_gaussian_narrowed_to_float32() {
        ...
        let reference_f32 = crate::filters::cast(&reference, PixelId::Float32).unwrap();
        assert_eq!(
