...
/// all ten [`crate::core::PixelId`] variants at runtime from a single generic
...
pub(crate) fn unary_apply<F: UnaryFunctor>(img: &Image, f: &F) -> Result<Image> {
    ...
    crate::filters::image_from_f64(img.pixel_id(), img.size(), img, &vals)
}
...
macro_rules! unary_functor {
    (
        ...
        pub fn $name(img: &Image $(, $p: $pt)*) -> Result<Image> {
            $crate::filters::functor::unary_apply(img, &($functor))
        }
        ...
        pub fn $name_in_place(img: Image $(, $p: $pt)*) -> Result<Image> {
            $crate::filters::functor::unary_apply_in_place(img, &($functor))
        }
    };
}
...
macro_rules! binary_functor {
    (
        ...
        pub fn $name(a: &Image, b: &Image) -> Result<Image> {
            $crate::filters::functor::binary_apply(a, b, &($functor))
        }
        ...
        pub fn $name_in_place(a: Image, b: &Image) -> Result<Image> {
            $crate::filters::functor::binary_apply_in_place(a, b, &($functor))
        }
    };
}
...
macro_rules! comparison_functor {
    (
        ...
        pub fn $name(a: &Image, b: &Image $(, $p: $pt)*) -> Result<Image> {
            $crate::filters::functor::comparison_apply(a, b, &($functor))
        }
    };
}
