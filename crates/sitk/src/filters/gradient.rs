...
//! in ITK — narrowing back to the output pixel type (`crate::filters::image_from_f64`)
...
//! borrowed [`crate::core::WindowView`]. Two costs the group used to carry are
...
use crate::filters::recursive_gaussian::{GaussianOrder, recursive_gaussian_f64_from_into};
...
/// [`Neighborhood::get`](crate::core::Neighborhood::get) re-derived on every
...
/// what lets them all read [`crate::core::WindowView`] — a *borrowed* window —
...
///
/// `pub(crate)`: [`crate::filters::watershed_classic::isolated_watershed`] needs them
...
/// the `R::from_f64` that [`crate::filters::image_from_f64`]'s narrowing map applied, to
...
///
/// `pub(crate)`: also reused by [`crate::filters::canny`], which applies this same
...
/// [`crate::core::Error::RequiresScalarPixelType`] on a vector or complex image.
///
...
/// [`Neighborhood::get`](crate::core::Neighborhood::get) addresses each by its
...
/// [`crate::core::Image::scalar_view`], which returns
/// [`crate::core::Error::RequiresScalarPixelType`]. That is the same error, on the
...
/// for every voxel. The window is now borrowed ([`crate::core::WindowView`]).
///
...
///
/// **Cascade order.** As with [`crate::filters::smoothing_recursive_gaussian`], ITK's actual
...
/// loop), via [`crate::core::Image::extract_component`]. A complex image falls
/// through to the scalar branch and is rejected there by `to_f64_vec`, the
/// same as [`crate::filters::smoothing_recursive_gaussian`].
///
...
/// [`crate::core::WindowView`] of the input's own pixels, with no copy of either.
///
