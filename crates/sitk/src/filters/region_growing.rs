...
//! narrowed through [`crate::core::Scalar::from_f64`], so magnitudes never
...
//! same accumulator rule [`vector_magnitude`](crate::filters::vector_magnitude)
//! follows.
//!
...
//! substitute. [`determinant_magnitude`](crate::core::matrix::determinant_magnitude)
...
//!
//! This port's own inverse ([`crate::core::matrix::invert`], Gauss-Jordan with
...
pub fn vector_confidence_connected(
    ...
    if !img.pixel_id().is_vector() {
        return Err(crate::core::Error::RequiresVectorPixelType(img.pixel_id()).into());
    }
    dispatch_scalar!(
        img.pixel_id(),
