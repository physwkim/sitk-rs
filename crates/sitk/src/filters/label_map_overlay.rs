...
//!   [`crate::filters::label_to_rgb::label_overlay`]. For every **integer** feature this
...
//! [`crate::filters::label_map::label_unique_label_map`], which reproduces that filter's
...
use crate::core::{Image, LabelMap, LabelObject, PixelId};

use crate::filters::error::{FilterError, Result};
use crate::filters::label_map::{
    label_unique_label_map, object_offsets, require_label_pixel_id, require_same_size, strides,
};
use crate::filters::label_to_rgb::{build_color_table, vector_image_from_f64};
use crate::filters::morphology::{StructuringElement, binary_dilate, binary_erode};
use crate::filters::{numeric_traits_max, quantize_to_pixel_type};
...
/// same `(p as i64) as u64` conversion [`crate::filters::label_to_rgb`] documents.
...
mod tests {
    use super::*;
    use crate::filters::label_to_rgb::DEFAULT_LABEL_COLORS;
    ...
    fn a_complex_label_map_cannot_be_constructed() {
        assert!(matches!(
            ...
            Err(crate::core::Error::RequiresIntegerPixelType(
