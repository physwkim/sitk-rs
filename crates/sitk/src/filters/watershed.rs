...
//! [`crate::filters::reconstruction::h_minima`], built on
...
//! [`crate::filters::connected_component`] — it is the same
...
pub fn morphological_watershed(
    ...
    let height = crate::filters::quantize_to_pixel_type(image.pixel_id(), level);
    if height < 0.0 {
    ...
    let minima_source = if height != 0.0 {
        crate::filters::reconstruction::h_minima(image, height, fully_connected)?.to_f64_vec()?
    } else {
        vals
    };
