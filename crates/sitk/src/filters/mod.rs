...
/// with the same [`crate::core::Error::RequiresScalarPixelType`] the read side
...
/// [`PixelId::is_scalar`](crate::core::PixelId::is_scalar), matching
...
pub(crate) fn image_from_f64(
    ...
    if !target.is_scalar() {
        return Err(crate::core::Error::RequiresScalarPixelType(target).into());
    }
...
/// [`crate::filters::scalar_to_rgb_colormap`] fix use exactly this idiom.
...
pub fn rescale_intensity(img: &Image, output_min: f64, output_max: f64) -> Result<Image> {
    #[cfg(feature = "cuda")]
    if let Some(gpu) = crate::cuda::try_rescale_intensity(img, output_min, output_max) {
        return Ok(gpu);
    }
...
pub fn rescale_intensity_cpu(img: &Image, output_min: f64, output_max: f64) -> Result<Image> {
    ...
    // the same `f64` the staged form computed. See `crate::core::fused`.
    let (lo, hi) = min_max_pixels(img)?.ok_or(FilterError::DegenerateRange)?;
    if lo == hi {
        return Err(FilterError::DegenerateRange);
    }
    let scale = (output_max - output_min) / (hi - lo);
    Ok(crate::core::map_pixels(img, img.pixel_id(), |v| {
...
/// image with [`crate::core::Error::RequiresScalarPixelType`]. `dispatch_scalar!`
...
///
/// The vector-consuming filters — [`crate::filters::vector`] and
/// [`crate::filters::displacement_field`] — are the exception, and they say so in their
...
/// input. [`crate::filters::dicom_orient`] is a third case: it accepts *either* scalar
/// or vector, dispatching on [`crate::core::PixelId::is_vector`] and, for a
...
mod vector_guard {
    use super::*;
    use crate::filters::morphology::StructuringElement;
    ...
    fn assert_requires_scalar(err: FilterError, expected: PixelId) {
        match err {
            FilterError::Core(crate::core::Error::RequiresScalarPixelType(id)) => {
    ...
    fn math_filter_rejects_a_vector_image() {
        let err = crate::filters::math::sqrt(&vector_image()).unwrap_err();
    ...
    fn binary_math_filter_rejects_a_vector_operand() {
        ...
        assert!(crate::filters::math::absolute_value_difference(&scalar, &vector_image()).is_err());
    }
    ...
    fn morphology_filter_rejects_a_vector_image() {
        ...
        let err =
            crate::filters::morphology::grayscale_dilate(&vector_image(), &kernel).unwrap_err();
        assert_requires_scalar(err, PixelId::VectorFloat32);
    }
    ...
    fn level_set_filter_rejects_a_vector_image() {
        let scalar = Image::new(&[2, 2], PixelId::Float32);
        let err = crate::filters::level_set::threshold_segmentation_level_set(
            &vector_image(),
            &scalar,
            0.0,
            1.0,
    ...
    fn pad_filter_rejects_a_vector_image() {
        let err = crate::filters::geometry::constant_pad(&vector_image(), &[1, 1], &[1, 1], 0.0)
            .unwrap_err();
        assert_requires_scalar(err, PixelId::VectorFloat32);
    }
    ...
    fn one_component_vector_image_is_still_rejected() {
        ...
        let err = crate::filters::math::sqrt(&img).unwrap_err();
        assert_requires_scalar(err, PixelId::VectorFloat32);
    }
    ...
    fn patch_based_denoising_rejects_a_vector_image() {
        ...
        let err = crate::filters::patch_based_denoising(&img, &Default::default()).unwrap_err();
        assert_requires_scalar(err, PixelId::VectorFloat32);
    }
...
/// [`PixelId::is_scalar`](crate::core::PixelId::is_scalar), and these cases pin
/// each route to the buffer against the category that used to bypass it.
///
/// The complex-consuming filters — [`crate::filters::complex`] — are the exception, and
...
mod complex_guard {
    ...
    fn math_filter_rejects_a_complex_image() {
        let err = crate::filters::math::sqrt(&complex_image()).unwrap_err();
    ...
    fn binary_math_filter_rejects_a_complex_operand() {
        let scalar = Image::new(&[2, 2], PixelId::Float32);
        assert!(
            crate::filters::math::absolute_value_difference(&scalar, &complex_image()).is_err()
        );
    }
    ...
    fn morphology_filter_rejects_a_complex_image() {
        let kernel = StructuringElement::ball(&[1, 1]);
        let err =
            crate::filters::morphology::grayscale_dilate(&complex_image(), &kernel).unwrap_err();
        assert_requires_scalar(err, PixelId::ComplexFloat32);
    }
    ...
    fn pad_filter_rejects_a_complex_image() {
        let err = crate::filters::geometry::constant_pad(&complex_image(), &[1, 1], &[1, 1], 0.0)
            .unwrap_err();
        assert_requires_scalar(err, PixelId::ComplexFloat32);
    }
    ...
    fn the_complex_filters_accept_a_complex_image() {
        assert!(crate::filters::complex::complex_to_real(&complex_image()).is_ok());
        assert!(crate::filters::complex::complex_to_modulus(&complex_image()).is_ok());
    }
}
