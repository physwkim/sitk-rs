...
//! [`box_mean`](crate::filters::box_mean)/[`box_sigma`](crate::filters::box_sigma) boundary
//! rule, not [`mean`](crate::filters::mean)/[`median`](crate::filters::median)'s
...
//! [`median`](crate::filters::median)'s `select_nth_unstable_by(len/2, ..)` (the
...
//! rule derived above, plus [`crate::filters::morphology::StructuringElement`] for the
...
//! crate's [`crate::core::NeighborhoodIterator`] (every existing
//! [`crate::core::BoundaryCondition`] substitutes *some* value for an
...
/// -- the same enumeration [`crate::filters::morphology`]'s own (private)
...
/// [`crate::core::NeighborhoodIterator::new`]'s own `RadiusMismatch` for the
...
pub fn rank(img: &Image, kernel: &StructuringElement, rank: f64) -> Result<Image> {
    ...
    if kernel.radius().len() != dim {
        return Err(crate::core::Error::RadiusMismatch { dimension: dim }.into());
    }
    let rank = (rank as f32).clamp(0.0, 1.0);
...
mod rank_tests {
    ...
    fn rank_rejects_a_kernel_of_the_wrong_dimension() {
        ...
        assert_eq!(
            err,
            FilterError::Core(crate::core::Error::RadiusMismatch { dimension: 2 })
        );
    }
