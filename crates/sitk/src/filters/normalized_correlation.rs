...
//!
//! This is a *different* filter from [`crate::filters::fft_correlation`]'s
...
//! `num`-sized box, [`crate::core::ZeroFluxNeumannBoundaryCondition`] at the
...
use crate::filters::geometry::same_physical_space;
...
/// [`crate::core::Neighborhood::values`] uses.
...
pub fn normalized_correlation(
    ...
    let output_id = crate::filters::real_pixel_id(image.pixel_id());
...
mod tests {
    ...
    fn non_scalar_image_is_rejected() {
        ...
        assert_eq!(
            err,
            crate::core::Error::RequiresScalarPixelType(PixelId::VectorFloat32).into()
        );
    }
    ...
    fn complex_image_is_rejected() {
        ...
        assert_eq!(
            err,
            crate::core::Error::RequiresScalarPixelType(PixelId::ComplexFloat32).into()
        );
    }

    /// Output pixel type follows [`crate::filters::real_pixel_id`]: a `Float32` input
...
/// borrowed [`crate::core::WindowView`] of the image's own pixels.
///
