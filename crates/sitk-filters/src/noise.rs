...
//!
//! Every filter here draws from one of the two RNGs in [`crate::random`]:
...
//! [`crate::random`]'s module doc comment for why that distinction matters);
...
//! [`clamp_cast`] mirrors this exactly (unlike [`sitk_core::Scalar::from_f64`]'s
//! plain saturating-truncating `as` cast used elsewhere in this crate).

use crate::error::Result;
use crate::random::{MersenneTwister, NormalVariateGenerator};
