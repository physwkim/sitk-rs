...
//! and [`fft_pad`].
//!
//! (`FFTShiftImageFilter` is the sixth, and lives in [`crate::fft_shift`].)
//!
...
//! `GenerateData` bodies on top of [`crate::fft`], which reimplements the
...
//!
//! Ledger: §2.93, §2.95, §3.33, §3.34, §4.1.

use sitk_core::{Complex as PixelComplex, Image, PixelId, Real};
...
/// yaml's own documentation still says "5 for VNL" — ledger §3.33.
...
/// through [`crate::fft`]'s Bluestein path. Use [`fft_pad`] first for a fast
/// length.
///
...
/// silently ignores whatever it finds. Ledger §2.95.
...
/// filter's documentation (ledger §2.93):
///
...
mod tests {
    ...
    /// reach the transform. Ledger §2.95.
    #[test]
    ...
    /// filter's own documentation. Ledger §2.93.
    #[test]
