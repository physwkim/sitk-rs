...
//! [`crate::n4_bias_field`] documents. For a `VectorFloat64` field the two agree
...
/// Errors:
///
/// - [`sitk_core::Error::RequiresVectorPixelType`] on a scalar image
...
pub(crate) fn require_displacement_field(img: &Image) -> Result<usize> {
    let id = img.pixel_id();
    if !id.is_vector() {
        return Err(sitk_core::Error::RequiresVectorPixelType(id).into());
    }
...
impl Field {
    ...
    pub(crate) fn from_image(img: &Image) -> Result<Self> {
        ...
        let inverse_direction =
            matrix::invert(img.direction(), dim).ok_or(sitk_core::Error::SingularDirection)?;
        Ok(Field {
            dim,
