...
fn main() {
    ...
    for &(size_name, dim) in SIZES {
        ...
        for op in OPS {
            ...
            // `doc/bench-spec.md` §"Thread configurations" defines a `gpu`
            // config. It is reported, always, as not run — because **the filter
            // API is CPU-only, by decision**, and the row records that rather
            // than leaving a hole in the schema.
            //
            // This is not "not implemented yet". `rescale_intensity` *was*
            // ported to CUDA and measured here: with the host allocation driven
            // to literally zero it still lost to the CPU by 1.4× at 256³ and
            // 2.2× at 512³, because a per-pixel op is bounded by the PCIe
            // transfer and the CPU never pays it. A dispatch path that always
            // loses is a dead branch, so the kernel and the dispatch were
            // removed; `doc/bench-results.md` keeps the measurement that says
            // why. `sitk-cuda` now serves exactly one thing, the registration
            // metric, where the same buffers are reused across hundreds of
            // iterations and the transfer amortizes.
            write_row(
                &mut out,
                &Row {
                    harness: "rust",
                    ...
                    max_rel_err: None,
                    skipped: Some(
                        "the filter API is CPU-only by decision: the GPU per-pixel op was \
                         measured slower than the CPU at every size and removed (see \
                         doc/bench-results.md); sitk-cuda serves the registration metric only"
                            .to_string(),
                    ),
                },
            );
