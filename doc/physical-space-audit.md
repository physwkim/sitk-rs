...
## Second correction: the binary in-place variants undercounted by 6 (95 → 101)

Found during the routing pass, when the workspace test run tripped on the newly-installed
checks. Six hand-written **binary in-place** entry points were never in any tally:
`squared_difference_in_place`, `absolute_value_difference_in_place`, `atan2_in_place`,
`binary_magnitude_in_place`, `divide_floor_in_place`, `pow_in_place` (`math.rs:399,418,433,447,476,497`).

They fell through **both** nets. Unlike the `binary_functor!` in-place forms
(`add_in_place` …), which were tallied from the macro, these are written out by hand — so the
macro count did not include them. And an in-place op takes its first operand **by value**
(`fn …(a: Image, b: &Image)`), leaving a **single `&Image`** in the signature — so the
original two-`&Image` text scan skipped them, and the "19 named" list carried only their
by-reference base names (`squared_difference` …), not the in-place siblings. The three
*ternary* in-place forms were counted because `(a: Image, b: &Image, c: &Image)` still shows
two `&Image`.

All six were **already routed** by construction: they run through
`two_image_f64_in_place`, which received the check in the same commit as the by-reference
path (`math.rs`, commit `63766ca`), and the direction is pinned by
`group2_math::binary_in_place_refuses_a_shifted_second_operand`. The correction is to the
*count*, not the code — the population is **101**, not 95, and the number to route was **80**,
not 74.

## VERIFIES — must check (85 entry points)

Grouped by upstream base; every one of them inherits the check unmodified.

**`BinaryGeneratorImageFilter` / `TernaryGeneratorImageFilter` functors (43: 19 named + 6
hand-written binary in-place + 12 generated by `binary_functor!` + 6 generated by
`comparison_functor!`)** — `add`,
...
`divide_floor`, `divide_real`, `pow`, their six hand-written in-place siblings
`squared_difference_in_place`, `absolute_value_difference_in_place`, `atan2_in_place`,
`binary_magnitude_in_place`, `divide_floor_in_place`, `pow_in_place` (see the second
correction below), `ternary_add`(+`_in_place`), `ternary_magnitude`
...
Of the **88** that must check (85 VERIFIES + 3 VERIFIES-AND-ADDS), **8 already do** —
...
image*. **80 entry points plus N4's confidence image are to be routed**, and
`masked_fft_normalized_correlation` keeps its own added size check beside the shared one.

## Routing landed

All 80 entry points plus N4's confidence image are routed through the single owner
`require_same_physical_space(primary, other, index)` (`geometry.rs`), surfaced as
`FilterError::PhysicalSpaceMismatch { index }`. Each coherent group is one commit; every
group carries a both-direction pin in `tests/physical_space_precondition.rs` (a VERIFIES
filter refuses a shifted origin, index-asserted; an EXEMPT/NOT-AN-INPUT neighbour still
accepts one). The routing was placed at shared helpers wherever one existed
(`two_image_f64_with_output`, `two_image_f64_in_place`, `check_same_size`, the fft
`check_inputs`, the demons `validate_verifying_image_pair`), so in-place and multi-entry
variants inherit the check by construction rather than site-by-site.

The **12 EXEMPT + 1 NOT-AN-INPUT** are deliberately left open, each with its upstream
citation in the sections above (`tile`'s non-calling override, `demons_registration`'s empty
`VerifyInputInformation`, Chan–Vese's non-input second image, etc.). Adding a check to any of
them would manufacture a divergence ITK does not have.

Landing the checks turned five pre-existing tests red: they had demonstrated "output takes
input[0]'s geometry" by feeding two inputs on *different* grids — the exact scenario the
inherited verifier refuses. Each was rewritten to feed congruent inputs (the propagation
intent survives; the refusal direction is pinned separately). `complex.rs`, `vector.rs`,
`displacement_field/invert.rs`, `fft_correlation.rs`, `demons/level_set_motion/tests.rs`
(commit `8d85e6c`).
